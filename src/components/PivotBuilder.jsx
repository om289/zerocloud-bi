...
const PivotBuilder = memo(function PivotBuilder({ activeTable, columns }) {
  ...
  const handleRunPivot = async () => {
    ...
    // Build DuckDB native PIVOT SQL:
    // PIVOT table ON columns USING function(val) GROUP BY rows
    const rowString = rows.join(', ');
    const colString = cols.join(', ');
    const sql = `PIVOT ${activeTable} ON ${colString} USING ${valFunc.toUpperCase()}(${valCol}) GROUP BY ${rowString};`;
  ...
  return (
    <div className="pivot-builder-grid">
      ...
      <div className="pivot-results-panel glass-panel">
        {error && (
